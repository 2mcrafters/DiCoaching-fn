...
const createLinkedContentRecursive = (text, publishedTerms) => {
    ...
    for (const { term, slug } of publishedTerms) {
        ...
        if (match) {
            ...
            const overlappingTerms = publishedTerms.filter(t => 
                matchedTerm.includes(t.term) && t.term !== matchedTerm
            );

            let linkContent;
            if (overlappingTerms.length > 0) {
                const innerParts = createLinkedContentRecursive(matchedTerm, overlappingTerms);
            ...
            const mainLink = (
                <Link key={slug} to={`/fiche/${slug}`} className="font-semibold text-primary hover:underline transition-colors duration-200">
                    {linkContent}
                </Link>
            );
            ...
            ].filter((value, index, self) => self.findIndex(t => t.slug === value.slug) === index);
