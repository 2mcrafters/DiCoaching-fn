...
const ModificationDetails = () => {
  ...
  useEffect(() => {
    ...
    if (mod) {
      ...
      const term = allTerms.find(t => t.id === mod.termId);
  ...
  const handleAction = (action) => {
    ...
    if (action === 'approve') {
      ...
      const updatedTerms = terms.map(term => {
        if (term.id === modification.termId) {
          ...
          // Ensure examples and sources are arrays
          if (finalChanges.examples) {
            finalChanges.examples = Array.isArray(finalChanges.examples) ? finalChanges.examples.filter(e => e.text?.trim()) : [];
          }
          if (finalChanges.sources) {
            finalChanges.sources = Array.isArray(finalChanges.sources) ? finalChanges.sources.filter(s => s.text?.trim()) : [];
          }
          return { ...term, ...finalChanges, updatedAt: new Date().toISOString() };
        }
        return term;
      });
    ...
    const updatedModifications = allModifications.map(m =>
      m.id === id ? { ...m, status: action === 'approve' ? 'approved' : 'rejected', reviewerId: user.id } : m
    );
    localStorage.setItem('coaching_dict_modifications', JSON.stringify(updatedModifications));

    toast({
      title: 'Action effectuée',
      description: `La modification a été ${action === 'approve' ? 'approuvée et appliquée' : 'rejetée'}.`,
    });
    navigate('/modifications');
  };

  if (loading) {
    return <div className="text-center py-20">Chargement des détails de la modification...</div>;
  }
  ...
  const canReview = (user.role === 'admin' || user.role === 'auteur') && modification.status === 'pending';


  return (
    <>
      <Helmet>
        <title>{modification?.termTitle ? `Détails de la Modification - ${modification.termTitle}` : 'Détails de la Modification'}</title>
        <meta name="description" content={`Détails de la modification proposée pour le terme ${modification.termTitle}.`} />
      </Helmet>
      <div className="min-h-screen creative-bg py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl">Modification pour: <Link to={`/fiche/${modification.termSlug}`} className="text-primary hover:underline">{modification.termTitle}</Link></CardTitle>
                <CardDescription className="flex items-center gap-4 pt-2">
                  <span className="flex items-center"><User className="h-4 w-4 mr-1" /> Proposé par: {proposer?.name || 'Inconnu'}</span>
                  <span className="flex items-center"><Calendar className="h-4 w-4 mr-1" /> Le {new Date(modification.createdAt).toLocaleString('fr-FR')}</span>
                  <Badge variant={modification.status === 'approved' ? 'success' : modification.status === 'rejected' ? 'destructive' : 'secondary'}>
                    {modification.status === 'approved' ? 'Approuvée' : modification.status === 'rejected' ? 'Rejetée' : 'En attente'}
                  </Badge>
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                {canReview ? (
                  ...
                  <div className="mt-8 pt-4 border-t text-center text-muted-foreground">
                    {modification.status === 'pending'
                        ? <p>Seuls les administrateurs et auteurs peuvent valider cette proposition.</p>
