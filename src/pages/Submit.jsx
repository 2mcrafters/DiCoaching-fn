...
const Submit = () => {
  ...
  const handleFormAction = (status) => {
    if (!formData.term?.trim() || (status !== 'draft' && (!formData.category || !formData.definition?.trim()))) {
      toast({
        title: "Erreur",
    ...
    const finalStatus = (user.role === 'auteur' || user.role === 'admin') ? 'published' : status;
    ...
    localStorage.setItem('coaching_dict_terms', JSON.stringify([...existingTerms, newTerm]));

    toast({
      title: finalStatus === 'published' ? "Terme publié !" : "Brouillon sauvegardé !",
      description: finalStatus === 'published' ? "Votre terme a été ajouté au dictionnaire." : "Votre brouillon a été sauvegardé. Vous pourrez le retrouver sur votre tableau de bord.",
    });
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Ajoutez un nouveau terme au dictionnaire collaboratif du coaching." />
      </Helmet>
      ...
      <div className="min-h-screen creative-bg py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-foreground mb-2">Soumettre un nouveau terme</h1>
              <p className="text-muted-foreground">Partagez vos connaissances avec la communauté du coaching</p>
            </div>

            <Card className="mb-8">
              <CardHeader>
                <CardTitle className="text-lg">Progression</CardTitle>
                <CardDescription>Complétez les champs pour améliorer la qualité de votre contribution</CardDescription>
              </CardHeader>
              <CardContent>
