...
const Fiche = () => {
  ...
  const loadData = () => {
    ...
    if (currentTerm) {
      ...
      const allComments = JSON.parse(
        localStorage.getItem("coaching_dict_comments") || "[]"
      );
  ...
  useEffect(() => {
    if (term) {
      const allLikes = JSON.parse(
        localStorage.getItem("coaching_dict_likes") || "{}"
      );
  ...
  const handleReportSubmit = (reason, details) => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify([...allReports, newReport])
    );
  ...
  const canEditDirectly =
    user && (user.role === "admin" || user.role === "auteur");
  const canProposeModification = user && !canEditDirectly;
  const authorBadge =
    author && author.id !== "user-api"
  ...
  return (
    <>
      <Helmet>
        <title>
          {term?.term
            ? `${term.term} - Dictionnaire du Coaching`
        ...
        >
          <div className="mb-8">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 text-muted-foreground text-sm">
              {author && (
                <Link
                  to={author.id !== "user-api" ? `/author/${author.id}` : "#"}
