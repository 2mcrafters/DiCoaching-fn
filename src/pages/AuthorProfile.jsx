...
import { User, FileText, Edit, MessageSquare, Facebook, Instagram, Linkedin, Twitter, Link as LinkIcon } from 'lucide-react';
...
const AuthorProfile = () => {
  ...
  useEffect(() => {
    ...
    const foundAuthor = allUsers.find(u => u.id === authorId && u.role === 'auteur');

    if (foundAuthor) {
      const termsAdded = allTerms.filter(term => term.authorId === authorId).length;
      const score = termsAdded * 10;
      const badge = getAuthorBadge(score);
      
      setAuthor({ ...foundAuthor, badge });
      setStats({ termsAdded, termsModified: 0 }); // Modification stats not tracked yet
    }
    setLoading(false);
  }, [authorId]);
  ...
  return (
    <>
      <Helmet>
        <title>{author?.name ? `Profil de ${author.name}` : 'Profil d\'auteur'} - Dictionnaire Collaboratif</title>
        <meta name="description" content={`Découvrez le profil et les contributions de ${author.name}.`} />
      </Helmet>
              ...
              <CardContent className="p-6 relative">
                <div className="flex flex-col sm:flex-row sm:items-end sm:space-x-5 -mt-20">
                  ...
                  <div className="mt-4 sm:mt-0 flex-1">
                    ...
                    <p className="text-md text-muted-foreground">{author.professionalStatus}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant={author.badge.variant} className="text-sm">
                        {React.cloneElement(author.badge.icon, { className: "h-3 w-3" })}
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                  <div className="p-4 bg-muted/50 rounded-lg">
                    ...
                    <p className="text-2xl font-bold mt-1">{stats.termsAdded}</p>
                    <p className="text-sm text-muted-foreground">Termes ajoutés</p>
                  </div>
                  <div className="p-4 bg-muted/50 rounded-lg">
                    <Edit className="h-6 w-6 mx-auto text-primary" />
                    <p className="text-2xl font-bold mt-1">{stats.termsModified}</p>
                ...
                <div className="mt-8">
                  <h2 className="text-xl font-bold text-foreground">Biographie</h2>
                  <p className="mt-2 text-muted-foreground whitespace-pre-wrap">{author.biography || "Aucune biographie fournie."}</p>
                </div>
                  ...
                  <div className="mt-8">
                    ...
                    <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                      {author.documents.map((doc, index) => (
                        <button onClick={() => handleDocumentClick(doc)} key={index} className="block p-4 border rounded-lg hover:bg-muted/50 text-center cursor-pointer">
                          <FileText className="h-8 w-8 mx-auto text-muted-foreground" />
                          <p className="mt-2 text-sm text-foreground truncate">{doc.title}</p>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {author.socials && author.socials.filter(s => s.url).length > 0 && (
                  <div className="mt-8">
                    <h2 className="text-xl font-bold text-foreground">Réseaux Sociaux</h2>
                    <div className="flex flex-wrap gap-4 mt-2">
                      {author.socials.filter(s => s.url).map((social, index) => (
                        <a href={social.url} key={index} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 border rounded-lg hover:bg-muted/50">
                          <SocialIcon network={social.network} />
                          <span className="text-sm font-medium">{social.customNetwork || social.network}</span>
                        </a>
                      ))}
                    </div>
                  </div>
