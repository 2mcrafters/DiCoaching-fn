...
router.get('/user/received-likes', authenticateToken, async (req, res) => {
  try {
    ...
    let authoredIds = [];
    if (hasTermes) {
      const rows = await db.query('SELECT id FROM termes WHERE author_id = ?', [userId]);
      authoredIds.push(...rows.map((r) => Number(r.id)));
    }
    if (hasTerms) {
      const rows = await db.query('SELECT id FROM terms WHERE author_id = ?', [userId]);
      authoredIds.push(...rows.map((r) => Number(r.id)));
    }

    // Deduplicate
    authoredIds = Array.from(new Set(authoredIds)).filter((v) => Number.isFinite(v));
    ...
    // 3) Hydrate users
    const likerIds = Array.from(new Set(likesRows.map((r) => Number(r.user_id)).filter((v) => Number.isFinite(v))));
    ...
    const termIds = Array.from(new Set(likesRows.map((r) => Number(r.term_id)).filter((v) => Number.isFinite(v))));
    if (termIds.length > 0 && hasTermes) {
      const tPlaceholders = termIds.map(() => '?').join(',');
      const tRows = await db.query(
        `SELECT id, terme AS name, slug, status FROM termes WHERE id IN (${tPlaceholders})`,
        termIds
      );
    ...
    const data = likesRows.map((like) => {
      ...
      const displayName =
        user.name || `${user.firstname || ''} ${user.lastname || ''}`.trim() || user.email || 'Utilisateur';
      return {
        id: like.like_id,
