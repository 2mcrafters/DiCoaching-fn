...
router.post("/register", async (req, res) => {
  try {
    ...
    // Notify admins if an author registration occurs
    try {
      if (role === "auteur" || role === "author") {
        ...
        for (const admin of admins) {
          await createNotification({
            ...
            message: `${firstname} ${lastname} a créé un compte auteur`,
            link: "/admin",
            meta: { actor: { id: result.insertId, firstname, lastname } },
          });
        }
      }
    } catch (nErr) {
      console.warn("[auth] notification warning:", nErr?.message);
    }
